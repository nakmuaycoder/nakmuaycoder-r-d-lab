...
![Forest Monitor Concept](forest_monitor.png){width=800px}
...
graph TD
    ...
    B --> C[Fetch risk levels JSON]
    B --> D[Parse Prefecture HTML for Forest IDs]
    C & D --> E[Map Risk Levels to Forest Names]
...
### 1. Scraping the Prefecture Data

The prefecture updates the risk levels on a dedicated platform: `risque-prevention-incendie.fr`. The scraper performs a dual-step operation:
* **HTML Parsing**: It uses `BeautifulSoup` to scrape the prefecture's main page for the department (e.g., `/13` for Bouches-du-Rhône) to extract the mapping of forest IDs to their human-readable names from the official table.
* **JSON Retrieval**: It fetches the detailed daily risk level data directly from the platform's JSON API (e.g., `import_data/{date}.json`). The script is designed with defensive fallback logic: it attempts to fetch tomorrow's date first (published in the evening) and falls back to today's date if tomorrow's data is not yet online.
...
To get the notification directly on my phone without needing a custom mobile app, I integrated the **Free Mobile SMS API** using the `freeMobileSMS` library. 
...
To make the system bulletproof, I refactored the formatting logic to use plain text ASCII tags:
```python
def format_status_message(self, date_label: str, is_mock: bool = False) -> str:
    ...
    return "\n".join(lines)
```
This ensures 100% delivery reliability while maintaining clean, legible lists directly on my lock screen.
...
The workflow is scheduled via cron to run every day at **20:00 (Paris time)**:
```yaml
on:
  schedule:
    # 18:00 UTC is 20:00 Paris time (during Daylight Saving Time)
    - cron: '0 18 * * *'
  workflow_dispatch: # Allows manual trigger
```

By specifying the timezone context carefully, we guarantee the check runs after the prefecture's 17:00 publication deadline. The workflow pulls configuration parameters and API credentials securely from GitHub Repository Secrets:
...
To keep this project maintainable and clean, I adopted several modern Python standards:
* **`uv`**: I use Astral's super-fast package manager `uv` to handle dependencies and run the environment. The virtual environment is managed easily via a simple `Makefile`.
* **`Ruff`**: For formatting and linting, ensuring code style is clean and consistent.
* **`detect-secrets`**: Integrated into pre-commit hooks to ensure no SMS API credentials ever leak into public commits.

```bash
# Running mock checks to test formatting
